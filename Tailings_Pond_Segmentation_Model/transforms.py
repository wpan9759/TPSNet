...
ImageToTensor = torchvision.transforms.ToTensor#将图像转为tensor


class MaskToTensor:#将数组格式的标签转为张量
...
class ConvertImageMode:#更改Image格式的mode
...
class JointCompose:#同时对图像与标签运用多种转换
...
class JointRandomVerticalFlip:
    ...
    def __call__(self, images, mask):
        ...
        if random.random() < self.p:
            return [v.transpose(Image.FLIP_TOP_BOTTOM) for v in images], mask.transpose(Image.FLIP_TOP_BOTTOM)#PIL中的transpose有几种模式
        else:
            return images, mask
